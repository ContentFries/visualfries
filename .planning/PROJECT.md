...
- ✓ `ReplaceSourceOnTimeCommand` shell (empty placeholder, never used) — existing

### Active

- [ ] `DeterministicMediaConfig` type added to `createSceneBuilder()` config
- [ ] `DeterministicFrameProvider` interface defined in runtime types
- [ ] `DeterministicFrameRequest` and `DeterministicFramePayload` types defined
- [ ] `DeterministicMediaManager` — stores config/provider, per-component cacheKey + resource cache, strict/diagnostics flags
- [ ] `DeterministicMediaFrameHook` — server-only hook for VIDEO/GIF that calls provider and writes pixiResource
- [ ] `ComponentDirector` routes VIDEO/GIF through deterministic hook in server mode before texture/display hooks
- [ ] `RenderFrameCommand` cache guard includes deterministic media fingerprint
- [ ] `SceneBuilder.setDeterministicFrameProvider()` and `getDeterministicFrameProvider()` methods
- [ ] `SceneBuilder.renderFrameRange()` helper with sink callback
- [ ] `ReplaceSourceOnTimeCommand` — implement as compatibility wrapper into `DeterministicMediaManager` (or delete and replace; decision gated at planning)
- [ ] Unit tests: cacheKey change → pixiResource update; same cacheKey → no recreate; null → native fallback; strict mode throws
- [ ] Integration tests: 3-video scene (normal, split-screen, fillBackgroundBlur); frame count equals `toFrame - fromFrame`; blob ≥30% faster than base64
- [ ] Diagnostics mode: provider hit/miss, cache hit ratio, per-frame latency — disabled by default, never throws
- [ ] All new API is backward-compatible; `deterministicMedia.enabled` defaults to `false`
...
**ReplaceSourceOnTimeCommand:** The command class file exists at `src/lib/commands/ReplaceSourceOnTimeCommand.ts` but is an empty, never-used placeholder. The spec calls for wiring it as a compatibility wrapper into `DeterministicMediaManager`; at planning time we will decide whether to implement it there or delete it and rebuild it correctly.
...
| ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | --------- |
| Provider-based API over sprite/internal patching                          | Clean interface boundary; lets cf-worker stay external; keeps Visualfries testable                       | — Pending |
| Blob-first transport recommendation                                       | 30%+ faster than base64 per experiment results; arraybuffer as alternative                               | — Pending |
| Deterministic hook short-circuits native media only when override present | Native path preserved as fallback; null provider response = native VIDEO playback                        | — Pending |
| Multi-tab parallelism stays external                                      | Visualfries remains single-scene; orchestration complexity belongs in cf-worker                          | — Pending |
| `ReplaceSourceOnTimeCommand` fate                                         | Empty placeholder — implement as DeterministicMediaManager wrapper or delete+replace; decide at planning | — Pending |
| cacheKey deduplication for frame cache guard                              | `RenderFrameCommand` must include deterministic fingerprint to bypass stale cached frames                | — Pending |
