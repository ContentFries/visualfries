...
export function collectDeterministicMediaRequirements(input: unknown): RenderMediaRequirement[] {
...
export function requiresDeterministicRender(input: unknown): boolean {
	return collectDeterministicMediaRequirements(input).length > 0;
}

export function resolveAgentRenderPlan(
	input: unknown,
	options: ResolveAgentRenderPlanOptions = {}
): AgentRenderPlan {
	...
	if (engine === 'browser-preview' && requiresDeterministicMedia) {
		const message =
			'Browser media rendering is preview-only for scenes with VIDEO/GIF components because native media seeking can produce stale or duplicated frames.';
