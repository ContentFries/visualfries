...
export const normalizeAndFilterRanges = (ranges: TimeRange[], boundary: TimeRange): TimeRange[] =>
	ranges
		.map((range) => ({
...
export const mergeOverlappingRanges = (ranges: TimeRange[]): TimeRange[] => {
	if (ranges.length < 2) return [...ranges];

	const sortedRanges = [...ranges].sort((a, b) => a.start - b.start);
	return sortedRanges.reduce<TimeRange[]>(
		(acc, range) => {
...
export const invertRemovedRanges = (
	...
	if (cursor < boundary.end) keptRanges.push({ start: cursor, end: boundary.end });
	return keptRanges;
};
...
export function resolveEffectiveRenderRanges(input: {
	...
	const fallbackRange: EffectiveRenderRange = {
