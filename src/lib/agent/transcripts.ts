import type { Subtitle, CompactWordTuple } from '../schemas/scene/subtitles.js';
...
function normalizeWord(word: TranscriptWordInput): CompactWordTuple | undefined {
...
function segmentText(segment: TranscriptSegmentInput): string {
...
function segmentTiming(segment: TranscriptSegmentInput): { start: number; end: number } | undefined {
	...
	const words = (segment.words ?? []).map(normalizeWord).filter(Boolean) as CompactWordTuple[];
...
export function normalizeTranscript(input: TranscriptInput): Subtitle[] {
	...
	return rawSegments
		.map((segment, index): Subtitle | undefined => {
			...
			if (!timing || !text) return undefined;
...
export function inferTranscriptDuration(subtitles: Subtitle[]): number {
...
function stripVttCueSetting(value: string): string {
	return value.trim().split(/\s+/, 1)[0] ?? value.trim();
}

function parseCueBlock(block: string, index: number): TranscriptSegmentInput | undefined {
	const lines = block
		.split(/\r?\n/)
		.map((line) => line.trim())
		.filter(Boolean);
	...
	if (timingIndex === -1) return undefined;
...
export function parseSubtitleText(input: string, format: TranscriptTextFormat = 'auto'): TranscriptInput {
	...
	return withoutHeader
		.split(/\r?\n\s*\r?\n/)
		.map(parseCueBlock)
		.filter(Boolean) as TranscriptSegmentInput[];
}
