import type { AnimationContext } from '../AnimationContext.js'; // Assuming this path

// Generic type for what the engine considers a 'target'
export type EngineTarget = any; // HTMLElement, object, string (selector), PIXI.DisplayObject, etc.
export type EngineTimeline = any; // Specific to the engine, e.g., gsap.core.Timeline
export type EngineStaggerOptions = any; // Specific to the engine's stagger implementation
...
export interface AnimationEngineAdaptor {
	...
	addTween(
		...
		targets: EngineTarget | EngineTarget[],
		method: 'to' | 'from' | 'fromTo' | 'set',
		vars: Record<string, any>,
		position?: string | number
	): number; // return the duration of the tween
	 ...
	 */
	resolveStagger(
		staggerDefinition: Record<string, any> | number,
	 ...
	 */
	// resolveDataReference(value: any, context: AnimationContext): any;
