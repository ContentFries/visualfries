...
} from './AnimationEngineAdaptor.ts';
import type { AnimationContext } from '../AnimationContext.ts';
import { get } from 'lodash-es';

// Specific type for GSAP timelines
type GsapEngineTimeline = gsap.core.Timeline;

export class GsapEngineAdaptor implements AnimationEngineAdaptor {
	...
	addTween(
		timeline: GsapEngineTimeline,
		targets: EngineTarget | EngineTarget[],
		method: 'to' | 'from' | 'fromTo' | 'set',
		vars: gsap.TweenVars, // GSAP's TweenVars is very flexible
	...
	resolveStagger(
		...
		_tweenDuration?: number // GSAP often handles duration within stagger context if 'amount' is used
	): EngineStaggerOptions {
		...
		if (type === 'fromData' && dataKey) {
			const timingsArray = context.getData(dataKey) as number[] | undefined;
			...
			return (index: number, _target: any, _targets: any) => {
