...
const openInput = (url: string): Input<UrlSource> =>
 ...
 */
export async function createMediabunnyVideoFrameProvider(
	options: MediabunnyVideoFrameProviderOptions
): Promise<DeterministicFrameProvider> {
	...
	try {
		...
		if (!track) throw new Error(`No video track found at ${options.url}`);
		if (!(await track.canDecode())) {
			throw new Error(`Primary video track cannot be decoded by this browser: ${options.url}`);
		}

		const sink = new VideoSampleSink(track as InputVideoTrack);
		...
		return {
			async getFrame(request) {
				if (disposed) throw new Error('MediaBunny video frame provider is disposed');
				if (request.componentId !== options.componentId) return null;
				if (request.fps !== options.fps) {
					throw new Error(
						`Video provider FPS mismatch: expected ${options.fps}, received ${request.fps}`
					);
				}
				if (request.frameIndex !== nextFrameIndex) {
					throw new Error(
						`MediaBunny video frames must be requested sequentially; expected ${nextFrameIndex}, ` +
							`received ${request.frameIndex}`
					);
				}
				if (nextFrameIndex >= maxFrames!) return null;
			...
			async releaseComponent(componentId) {
				if (componentId === options.componentId) await dispose();
			},
			destroy: dispose
		};
	} catch (error) {
...
export async function mixMediabunnyAudioTracks(
	options: MixMediabunnyAudioTracksOptions
): Promise<AudioBuffer> {
	...
	const context = new OfflineAudioContext(
		channels,
		Math.ceil(options.duration * sampleRate),
		sampleRate
	);

	for (const trackOptions of options.tracks) {
		if (trackOptions.muted || (trackOptions.volume ?? 1) <= 0) continue;
		...
		if (!Number.isFinite(endAt) || endAt <= trackOptions.startAt) continue;

		const input = openInput(trackOptions.url);
		try {
			const track = await input.getPrimaryAudioTrack();
			if (!track) throw new Error(`No audio track found at ${trackOptions.url}`);
			if (!(await track.canDecode())) {
				throw new Error(`Primary audio track cannot be decoded by this browser: ${trackOptions.url}`);
			}

			const sink = new AudioBufferSink(track as InputAudioTrack);
			...
			for await (const wrapped of sink.buffers()) {
				...
				if (relativeTimestamp >= activeDuration) break;
