...
export class TextAppearanceStyleProcessor implements StyleProcessor<TextAppearance | undefined> {
	public process(textAppearance: TextAppearance | undefined): Record<string, any> {
		let styles: HtmlElementStyle = {};
		if (!textAppearance) return styles;

		if (textAppearance.fontFamily) styles.fontFamily = textAppearance.fontFamily;
		if (textAppearance.fontWeight) styles.fontWeight = String(textAppearance.fontWeight);
		if (textAppearance.textAlign) styles.textAlign = textAppearance.textAlign;
