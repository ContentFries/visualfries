...
export class HtmlToCanvasHook implements IComponentHook {
	...
	#sprite!: Sprite;
	#lastHtml: string | undefined = undefined;

	private svgBase: string | null = null;
	private svgEnd: string | null = null;
	...
	async #svgToTexture(svgString: string): Promise<Texture> {
		// seems fastest
	...
	async #renderSvg() {
		...
		const { width, height } = this.state;
		if (!this.svgBase) {
			const { base, content, end } = await svgGenerator.generateSVG(
				this.#htmlEl,
				this.#context.data.appearance.text as unknown as TextAppearance,
				width,
				height,
