...
export class SubtitlesHook implements IComponentHook {
	...
	#initialized = false;
	#subtitles: Subtitle[] = [];
	#currentSubtitle: Subtitle | undefined = undefined;
	...
	async #handleSetup() {
		...
		const data = this.#context.data as SubtitleComponent;
		const source = data.source;
		const assetId =
			data.source && typeof data.source.assetId === 'string' ? data.source.assetId : '';
