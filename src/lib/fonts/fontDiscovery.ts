...
const normalizeFamily = (value: unknown): string | null => {
...
const parseWeightsFromVariants = (variants: unknown): number[] => {
...
const parseConfiguredFamilyDescriptor = (value: string, fallbackFamily: string) => {
...
const parseConfiguredFonts = (configuredFonts: FontType[]): ParsedConfiguredFont[] => {
	const parsedFonts: ParsedConfiguredFont[] = [];
	...
	for (const configuredFont of configuredFonts) {
		...
		const parsedFamilyDescriptor = parseConfiguredFamilyDescriptor(
			configuredFamilyDescriptor,
			fallbackFamily
		);
...
const createConfiguredLookup = (configuredFonts: ParsedConfiguredFont[]) => {
	const lookup = new Map<string, ParsedConfiguredFont>();
	for (const configuredFont of configuredFonts) {
...
const collectComponentTextVariants = (
	component: any,
	configuredLookup: Map<string, ParsedConfiguredFont>,
	output: Map<string, FontVariantDescriptor>
) => {
	...
	const source = (textAppearance.fontSource?.source ?? configMatch?.source ?? 'google') as
		...
		| 'custom';
	const fileUrl =
		normalizeFamily(textAppearance.fontSource?.fileUrl) ?? configMatch?.fileUrl ?? undefined;
...
export const discoverRequiredFontVariants = (
	sceneData: Scene,
