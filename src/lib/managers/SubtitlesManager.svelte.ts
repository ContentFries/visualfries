...
// Default settings
const DEFAULT_SETTINGS: SceneSubtitlesSettings = {
	punctuation: true,
	mergeGap: 0.3
};
...
function generateWordsFromText(
	text: string,
	startTime: number,
	endTime: number
): CompactWordTuple[] {
...
function validateAndAdjustWordTiming(
	originalWords: CompactWordTuple[],
	text: string,
	startTime: number,
	endTime: number
): CompactWordTuple[] {
	...
	if (
		textWords.length !== wordTexts.length ||
...
function findNaturalSplitIndex(text: string, targetChars: number, tolerance = 12): number {
	const length = text.length;
	if (length <= targetChars) return -1;
	...
	const advanceAfterBreak = (i: number) => {
		let j = i + 1;
	...
	const punctuationOnly = (s: string) => /^[\s,.!?:;]+$/.test(s);
	const validSplit = (pos: number) => {
		...
		if (punctuationOnly(first) || punctuationOnly(second)) return false;
		// avoid starting next with punctuation when possible
		if (/^[,.!?:;]/.test(second)) return false;
		return true;
	};
	...
	for (let i = Math.min(targetChars, windowEnd); i >= windowStart; i--) {
		if (strong.has(text[i])) {
			...
			if (validSplit(splitAt)) return splitAt;
		}
	}
	...
	for (let i = targetChars; i >= 1; i--) {
		if (text[i - 1] === ' ' && validSplit(i)) return i;
	}
...
function buildSubtitlesManager(
	...
	let index = $state<Record<string, Record<string, Record<string, Subtitle>>>>({});

	// Settings state
	const sceneSubtitlesSettings = get(sceneData, 'settings.subtitles', {});
	let settings = $state<SceneSubtitlesSettings>({ ...DEFAULT_SETTINGS, ...sceneSubtitlesSettings });

	const configuredMergeGap =
		typeof settings?.mergeGap === 'number' ? settings.mergeGap : (DEFAULT_SETTINGS.mergeGap ?? 0);
	...
	function markSubtitleClean(subtitleId: string) {
	...
	function markLanguageDirty(assetId: string, lang: string) {
		const key = `${assetId}:${lang}`;
		validatedCollections.delete(key);
	}

	// Cache validated collection for an asset/language
	function cacheValidatedCollection(assetId: string, lang: string, collection: Subtitle[]) {
		const key = `${assetId}:${lang}`;
		validatedCollections.set(key, collection);
	}

	// Get cached collection if available
	function getCachedCollection(assetId: string, lang: string): Subtitle[] | undefined {
		const key = `${assetId}:${lang}`;
		return validatedCollections.get(key);
	}

	// Derived from index - converts to collection format with word timing validation
	const subtitlesData = $derived.by<Record<string, SubtitleCollection>>(() => {
		const result: Record<string, SubtitleCollection> = {};
		for (const [assetId, assetIndex] of Object.entries(index)) {
			const collection: SubtitleCollection = {};
			for (const [lang, langIndex] of Object.entries(assetIndex)) {
				...
				const validatedCollection = subsInOrder.map((subtitle, i) => {
					...
					let adjustedWords: CompactWordTuple[] = [];
					if (subtitle && validatedSubtitles.has(subtitle.id)) {
						// Subtitle is clean, use existing words
						adjustedWords = (subtitle?.words || []) as CompactWordTuple[];
					} else if (subtitle) {
						// Subtitle is dirty, validate and cache
						adjustedWords = validateAndAdjustWordTiming(
							(subtitle.words || []) as CompactWordTuple[],
							subtitle.text,
							subtitle.start_at,
							endAt
						);
	...
	const sorted = $derived.by(() => {
		const result: Record<string, Record<string, Subtitle[]>> = {};
		for (const [assetId, collection] of Object.entries(subtitlesData)) {
			const assetSorted: Record<string, Subtitle[]> = {};
	...
	function initializeData() {
		if (Object.keys(subtitles).length > 0) {
			...
			if (Array.isArray(firstValue)) {
				...
				processLegacySubtitles(subtitles as Record<string, Subtitle[]>);
			} else {
				// New format: Record<string, SubtitleCollection>
				processSubtitleCollections(subtitles as Record<string, SubtitleCollection>);
			}
		}
	}
	...
	function recomputeIndex() {
		const newIndex: Record<string, Record<string, Record<string, Subtitle>>> = {};
		for (const [assetId, assetIndex] of Object.entries(index)) {
			const newAssetIndex: Record<string, Record<string, Subtitle>> = {};
			...
			for (const [lang, langIndex] of Object.entries(assetIndex)) {
				...
				const newLangIndex: Record<string, Subtitle> = {};
	...
	function recomputeIndexForLanguage(assetId: string, lang: string) {
	...
	function processSubtitleCollections(collections: Record<string, SubtitleCollection>) {
		const newIndex: Record<string, Record<string, Record<string, Subtitle>>> = {};

		for (const [assetId, collection] of Object.entries(collections)) {
			const assetIndex: Record<string, Record<string, Subtitle>> = {};
			for (const [lang, subs] of Object.entries(collection)) {
				const langIndex: Record<string, Subtitle> = {};
				for (const sub of subs) {
					langIndex[sub.id] = normalizeSubtitle(sub) as Subtitle;
				}
	...
	function processLegacySubtitles(legacySubtitles: Record<string, Subtitle[]>) {
		// Group by assetId, handling both assetId and assetId-languageCode patterns
		const assetGroups: Record<string, Record<string, Subtitle[]>> = {};
		...
		for (const [key, subtitles] of Object.entries(legacySubtitles)) {
			...
			assetGroups[assetId][languageCode] = subtitles.map(
				(sub) => normalizeSubtitle(sub) as Subtitle
			);
		}
	...
	function initializeSceneAssetSubtitles(sceneData: Scene) {
			...
			if (asset.subtitles && asset.subtitles.length > 0) {
				...
				for (const subtitleEntry of asset.subtitles) {
					...
					if (subtitleEntry.subtitles) {
						const languageCode = subtitleEntry.language_code || 'default';
						languageGroups[languageCode] = subtitleEntry.subtitles.map(
							(sub) => normalizeSubtitle(sub) as Subtitle
						);
					}
				}
				...
				if (Object.keys(languageGroups).length > 0) {
					...
					if (existingIndex && Object.keys(existingIndex).length > 0) {
						console.warn(
							`SubtitlesManager: Replacing existing subtitles for asset ${assetId} with scene data subtitles`
						);
					}
	...
	async function fetchSubtitlesFromUrl(url: string): Promise<Subtitle[] | null> {
		try {
			...
			if (!response.ok) {
				console.warn(
					`SubtitlesManager: Failed to fetch subtitles from ${url}: ${response.status} ${response.statusText}`
				);
				return null;
			}
	...
	function isValidSubtitlesArray(data: any): boolean {
	...
	function getDefaultLanguage(assetId: string): string {
		return 'default'; // or logic to find first available
	}

	// Public interface
	return {
		get data(): Record<string, SubtitleCollection> {
		...
		getAssetSubtitlesForSceneData(
			assetId: string
		): Array<{ language_code: string; subtitles: Subtitle[] }> {
			const collection = subtitlesData[assetId];
			if (!collection) {
				return [];
			}

			const result: Array<{ language_code: string; subtitles: Subtitle[] }> = [];
		...
		getAssetSubtitles(assetId: string) {
			if (!subtitlesData[assetId]) {
				console.warn(
					'-- subtitlesData does not have assetId',
					assetId,
					subtitlesData,
					index,
					subtitles
		...
		getText(assetId: string, language = '') {
		...
		setAssetSubtitles(assetId: string, subtitles: Subtitle[]) {
			const assetIndex: Record<string, Record<string, Subtitle>> = {};
			const langIndex: Record<string, Subtitle> = {};
		...
		replaceCollection(assetId: string, newCollection: SubtitleCollection, language = '') {
			const assetIndex: Record<string, Record<string, Subtitle>> = {};
		...
		splitByChars(assetId: string, language = '', maxChars: number) {
			const lang = language || getDefaultLanguage(assetId);
			if (!assetId || maxChars <= 0) return;
			...
			for (let i = 0; i < list.length; i++) {
				let sub = list[i];
				if (!sub || typeof sub.text !== 'string') continue;

				while (sub.text.length > maxChars) {
					if (++ops > maxOps) {
						console.warn(
							'SubtitlesManager.splitByChars: aborting due to excessive operations (split)'
						);
						return;
					}
					...
					if (splitAt <= 0 || splitAt >= sub.text.length) break;
				...
				if (trimmedLen < threshold) {
					...
					if (next) {
						...
						if (combinedLen <= Math.round(maxChars * 1.15) && gap <= 0.7) {
							if (++ops > maxOps) {
								console.warn(
									'SubtitlesManager.splitByChars: aborting due to excessive operations (merge)'
								);
								return;
							}
		...
		updateSubtitleText(assetId: string, subtitleId: string, newText: string, language = '') {
			const lang = language || getDefaultLanguage(assetId);
			const sub = index[assetId]?.[lang]?.[subtitleId];
			if (!sub) return;

			const oldWords = (sub.words || []) as CompactWordTuple[];
			// Filter out empty words from split - use safe filtering
			const newWords = newText.split(' ').filter((word) => word.trim() !== '');

			let newWordsArray: CompactWordTuple[];
			...
			if (newText.trim() === '' || newWords.length === 0) {
				...
				newWordsArray = newWords.map((word, i) => {
					...
					if (!oldWord) {
						...
						return [word, sub.start_at, sub.end_at] as CompactWordTuple;
					}
					...
					const tuple: CompactWordTuple = [word, startTime, endTime];
		...
		getSubtitle(assetId: string, timeOrId: string | number, language = ''): Subtitle | undefined {
			const lang = language || getDefaultLanguage(assetId);
			...
			if (typeof timeOrId === 'string') {
				...
				let low = 0,
					high = sortedSubs.length - 1;
				while (low <= high) {
					const mid = Math.floor((low + high) / 2);
					const sub = sortedSubs[mid];
					if (timeOrId >= sub.start_at && timeOrId <= sub.end_at) return sub;
					if (timeOrId < sub.start_at) high = mid - 1;
					else low = mid + 1;
				}
				return undefined;
			}
		...
		setStart(assetId: string, subtitleId: string, start: number, language = '') {
			const lang = language || getDefaultLanguage(assetId);
		...
		setEnd(assetId: string, subtitleId: string, end: number, language = '') {
			const lang = language || getDefaultLanguage(assetId);
			const sub = this.getSubtitle(assetId, subtitleId, language);
		...
		splitSubtitle(assetId: string, subtitleId: string, language = '', splitAt: number) {
			const lang = language || getDefaultLanguage(assetId);
			const sub = this.getSubtitle(assetId, subtitleId, language);
			...
			if (splitAt <= 0 || splitAt >= sub.text.length) {
				console.warn(
					`SubtitlesManager: Invalid split position ${splitAt} for subtitle with length ${sub.text.length}`
				);
				return;
			}

			const words = (sub.words || []) as CompactWordTuple[];
			...
			for (let i = 0; i < words.length; i++) {
				...
				if (!wordTuple || wordTuple.length < 3) continue; // Skip invalid tuples

				const word = wordTuple[0];
				if (typeof word !== 'string') continue; // Skip invalid word text
			...
			if (splitWordIndex === -1) {
				...
				for (let i = 0; i < words.length; i++) {
					const wordTuple = words[i];
					if (!wordTuple || wordTuple.length < 3) continue;
				...
				const firstWordPart: CompactWordTuple = [
					originalWord.substring(0, splitCharInWord),
					originalStartTime,
					wordSplitTime
				];

				const secondWordPart: CompactWordTuple = [
		...
		mergeSubtitles(
			...
			if (!sourceSub) {
				console.warn(
					`SubtitlesManager: Subtitle ${sourceSubtitleId} not found for asset ${assetId}`
				);
				return;
			}
			...
			if (sourceIndex === -1) {
				console.warn(
					`SubtitlesManager: Source subtitle ${sourceSubtitleId} not found in sorted list`
				);
				return;
			}

			let targetSub: Subtitle | undefined;
			let subtitleToDeleteId: string;
			let survivingSubtitle: Subtitle;
			...
			if (mergeTo === 'start') {
				...
				survivingSubtitle = {
					...
					words: [...(sourceSub.words || []), ...(targetSub.words || [])] as CompactWordTuple[]
				};
			} else {
				// mergeTo === 'end'
				...
				if (sourceIndex === 0) {
					console.warn(
						`SubtitlesManager: No previous subtitle to merge with for ${sourceSubtitleId}`
					);
					return;
				}
				targetSub = sortedSubs[sourceIndex - 1];
				...
				survivingSubtitle = {
					...
					words: [...(targetSub.words || []), ...(sourceSub.words || [])] as CompactWordTuple[]
				};
			}
		...
		addNewSubtitleAfter(assetId: string, subtitleId: string, language = '') {
			const lang = language || getDefaultLanguage(assetId);
			...
			if (gap <= minGap) {
				console.warn(
					`SubtitlesManager: Gap too small (${gap}s) to insert new subtitle after ${subtitleId}`
				);
				return;
			}
...
export class SubtitlesManager {
	private builder: ReturnType<typeof buildSubtitlesManager>;
	...
	constructor(cradle: {
		...
		this.builder = buildSubtitlesManager(
			cradle.timeManager,
			cradle.eventManager,
			cradle.sceneData,
			cradle.subtitles
	...
	updateSubtitleText(subtitleId: string, newText: string) {
	...
	updateSubtitleProps(subtitleId: string, props: Partial<Subtitle>) {
		if (!this.assetId || !this.language) {
			console.warn('SubtitlesManager: Asset ID or language is not set');
			return;
		}
	...
	splitSubtitle(subtitleId: string, splitAt: number) {
		if (!this.assetId || !this.language) {
			console.warn('SubtitlesManager: Asset ID or language is not set');
			return;
		}
	...
	mergeSubtitles(sourceSubtitleId: string, mergeTo: 'start' | 'end') {
		if (!this.assetId || !this.language) {
			console.warn('SubtitlesManager: Asset ID or language is not set');
			return;
		}
	...
	// TODO fix, refactor
	setTimesForSubtitleInAssetAndLanguage(
		assetId: string,
		language: string,
		subtitleId: string,
		start?: number,
	...
	setStart(subtitleId: string, start: number) {
		if (!this.assetId || !this.language) {
			console.warn('SubtitlesManager: Asset ID or language is not set');
			return;
		}
	...
	setEnd(subtitleId: string, end: number) {
		if (!this.assetId || !this.language) {
			console.warn('SubtitlesManager: Asset ID or language is not set');
			return;
		}
	...
	addNewSubtitleAfter(subtitleId: string, newText: string) {
		if (!this.assetId || !this.language) {
			console.warn('SubtitlesManager: Asset ID or language is not set');
			return;
		}
	...
	public findTextChunkTiming(
		...
		const normalizedSearchText = (
			options.caseSensitive ? searchText : searchText.toLowerCase()
		).trim();
