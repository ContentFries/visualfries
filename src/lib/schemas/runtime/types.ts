...
// Define separate event interfaces for better organization
...
// Combine all event definitions into a single type
export type EventMap = StateEvents &
	TimelineEvents &
	RenderEvents &
	PlaybackEvents &
	LayerEvents &
...
export interface MediaComponent {
	...
	autoSeek: () => Promise<void>; // auto seek given the current scene timeline time
}
...
export type ComponentProps = ComponentBase & {
	...
	// setKeyframes(keyframes: SceneLayerComponentKeyframeData[]): void;
...
export interface PixiComponent {
	displayObject: Container; // TODO move elsewhere
}
...
export interface Component {
	...
	readonly displayObject: Container | undefined; // rethink this
...
export interface ComponentContext {
	...
	isActive: boolean; // timeline time is in the component time
...
export type ComponentRefreshType =
	| 'refresh' // Full refresh
	| 'refresh:content' // Text content, video source changes
	| 'refresh:config' // Position, size, style properties
	| 'refresh:metadata' // Font settings, split screen settings
	| 'refresh:animation' // Animation settings
	| 'refresh:audio'; // Mute/unmute

export type HookType =
	| 'setup' // Initial setup
	| 'update' // Every render tick
	| 'destroy'
