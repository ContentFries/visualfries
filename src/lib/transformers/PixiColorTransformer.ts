...
interface PixiColorResult {
	type: 'solid' | 'gradient';
	color?: number; // PIXI color as number
	texture?: PIXI.Texture; // For gradients
	width?: number;
	height?: number;
}
...
export class PixiColorTransformer {
	...
	static #extractColor(background: BackgroundShape): ColorType | null {
		...
		if (
			...
			return background as ColorType;
		}

		// If it's a BgShape object
		if (typeof background === 'object' && 'enabled' in background) {
			const bgShape = background as BgShape;
			if (!bgShape.enabled) return null;
	...
	static #createGradientCanvas(
		...
		const colorsWithStops = gradient.colors.map((col: string, index: number) => {
	...
	static isGradient(background: BackgroundShape): boolean {
		...
		return (
			typeof resp.data === 'object' && (resp.data.type === 'linear' || resp.data.type === 'radial')
		);
	}
}
